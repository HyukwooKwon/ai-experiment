...
# ✅ 환경 변수 강제 로드 (Render에서 .env 파일이 자동 적용되지 않을 수도 있음)
dotenv_path = os.path.join(os.path.dirname(__file__), ".env")
load_dotenv(dotenv_path)

# ✅ 현재 배포 환경 감지 (GPT-4 전용인지, GPT-3.5 전용인지)
DEPLOY_ENV = os.getenv("DEPLOY_ENV", "both")  # 기본값은 gpt4 & gpt3.5 모두 지원
...
# ✅ DEPLOY_ENV에 따라 해당 모델만 지원하도록 업체 목록을 자동 필터링
if DEPLOY_ENV == "gpt4":
    ALLOWED_COMPANIES = [company for company, model in COMPANY_AI_MODELS.items() if model == "gpt-4-turbo"]
elif DEPLOY_ENV == "gpt3.5":
    ALLOWED_COMPANIES = [company for company, model in COMPANY_AI_MODELS.items() if model == "gpt-3.5-turbo"]
else:
    ALLOWED_COMPANIES = list(COMPANY_AI_MODELS.keys())  # 모든 업체 허용
